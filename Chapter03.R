...
# カイ二乗検定を実行する
dat2 %>% chisq.test
# chisq.test(dat2) ←こうしても同じ結果を得られる
...
# グラフ（積み上げバープロット）を描く
library(ggplot2)
table1 %>% as.data.frame %>% ggplot(aes(x = 立場, y = Freq, fill = 回答6)) + geom_bar(stat = "identity") + ylab("人数") + theme_bw(base_family = "HiraKakuPro-W3")
# + scale_fill_grey(start = 0.4, end = 0.8) ←はグラフをモノクロにするオプション
# パイプ演算子を使わないで同じプロットを作成
# ggplot(table1, aes(x = 立場、y = Freq, fill = 回答6)) + geom_bar(stat = "idendity")
